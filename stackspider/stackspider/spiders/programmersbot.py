...
class ProgrammersbotSpider(Spider):
    ...
    async def parseContent(self, response):
        ...
        """
        list_of_companies = response.css(
            "section[class=jobs__container] a::attr(href)"
        ).getall()
        ...
        for company_card_url in list_of_companies:
            ...
            yield Request(
                url, callback=self.parseCompanyContent, errback=self.errback_httpbin
            )
    ...
    def parseCompanyContent(self, response):
        ...
        for job_card_url in list_of_job_positions:
            ...
            request = Request(
                url, callback=self.parseJobCardContent, errback=self.errback_httpbin
            )
            yield request
